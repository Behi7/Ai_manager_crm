    ...
    async def process_lead_after_debounce(self, account_id_str: str, lead_id_str: str):
        ...
        try:
            ...
            async with AsyncSessionLocal() as session:
                ...
                # Получаем актуальные данные сделки из amoCRM
                lead_amo_data = await amocrm_client.get_lead(subdomain, access_token, amo_lead_id)

                # Проверка фильтра воронок: если в аккаунте включены определенные воронки,
                # проверяем, что сделка находится в одной из них
                ...
                db_messages = (await session.execute(history_stmt)).scalars().all()
                recent_messages = db_messages[-20:]
                ...
                if comm_resp.is_handover_requested or is_stuck_threshold:
                    ...
                    reply_field_id = account.ai_reply_field_id or 1000463
